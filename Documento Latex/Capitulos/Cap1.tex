...
En \textit{machine learning}, una gran variedad de modelos contienen ciertos parámetros que no llegan a ser prendidos unicamente con el análisis de los datos, lo que lleva a que deban ser definidos antes de empezar el entrenamiento. Estos parámetros son conocidos como hiperparámetros. \\  

Las estrategias más se emplean a la hora de probar diferentes combinaciones de hiperparámetros y evaluarlas mediante métodos de validación, son \textit{grid search y random search} \cite{liashchynskyi2019grid}. En la estrategia de \textit{grid search}, los valores estudiados de cada hiperparámetro, deben ser distribuidos uniformemente dentro de un rango que será establecido por el analista. En la estrategia de \textit{random search}, los datos son aleatorios dentro del rango definido, como se muestra en la Figura \ref{fig:Hiperparámetros grid search y random search}.
...
En general las dos estrategias anteriormente mencionadas son válidas y los resultados obtenidos son buenos, sin embargo estas estrategias comparten la misma insuficiencia y es que ninguna de las dos tiene en cuenta los resultados obtenidos hasta el momento, Esto trae como consecuencia que estas estrategias no estén focalizadas en la búsqueda de las regiones de mayor interés y así evitar las regiones innecesarias.\\


La optimización bayesiana es otro método utilizado para la búsqueda de los hiperparámetros de un modelo, esta estrategia fue definida por \textit{Peter Frazier (2018)} \cite{frazier2018tutorial} como \textit{"crear un modelo probabilístico en el que el valor de la función objetivo es la métrica de validación del modelo, en este caso, la precisión"}. Con este método, se logra que la búsqueda sea redirigida en cada iteración hacia las regiones de mayor interés. Esto con el fin de reducir el número de combinaciones de los hiperparámetros con los que el modelo es evaluado.\\
...
La arquitectura \textit{ResNet} (Residual Net), fue propuesta por un equipo de Microsoft Research liderado por \textit{Kaiming He} \cite{he2016deep}, en donde se propone la implementación de de la red ResNet basándose en las conexiones residuales. El termino conexión residual es implementado para describir la conexión que existe entre el output de las capas nuevas con el output de las capas anteriores \cite{18}. 

Las redes residuales pueden variar su cantidad de capas efectivas, es por esto que es necesario introducir el concepto de bloque residual, el cual es presentado n la Figura \ref{fig:Bloque}.
