...
Este es una técnica que intenta expandir la entrada insertando espacios entre los huecos de sus elementos consecutivos. Como la convolución normal, pero con una diferencia, que ignora
píxeles para cubrir más área de entrada, brindándole un campo de visión más amplio.
Al mismo costo computacional, a través de un parámetro llamado coeficiente de expansión
\textit{l}, que indica el ancho de la entrada
 \cite{prakharr0y}. La Figura \ref{fig:cd} muestra un ejemplo de la implementación de la convolución dilatada.
...
Este término alude a la cercanía entre la eficacia de una medición y el valor real. La forma de calcularse se muestra en la Ecuación \ref{eq:Exactitud}.
...
Este término por otro lado consiste en lo mal o bien que actúa un modelo luego de cada iterización de optimización, expresado en la Ecuación \ref{eq:perdida}.

\begin{equation}
	Losses=\frac{FP+FN}{VP+VN+FP+FN}
	\label{eq:perdida}
\end{equation}
...
La arquitectura general de una red neuronal convolucional (\textit{RNC}) se muestra en la Figura \ref{fig:cnnarchitecture}, que consiste en varias capas de procesamiento de imágenes en donde se realizan convoluciones () \cite{o2015introduction}
 ...
 A common form of CNN architecture in which convolutional layers are
