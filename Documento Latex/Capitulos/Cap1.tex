...
\section{Clasificación De Papa En Colombia}
...
El modelo se compone de $5$ capas convolucionales y $3$ capas densas, donde las capas convolucionales están normalizadas por lotes de datos. El $kernel$ (Máscara aplicada en la convolución) tiene dimensiones diferentes que se distribuyen en \textit{11 x 11} en la primera capa, \textit{5 x 5} en la segunda y \textit{3 x 3} en las demás. La primera, cuarta y quinta capa convolucional están seguidas por una capa \textit{Max pooling} con dimensiones de \textit{3 x 3}, que posee un \textit{stride} de $2$.\\


Las capas convolucionales y de \textit{Max pooling} son seguidas por tres capas densas, en donde las dos primeras poseen $4096$ neuronas cada una y la ultima capa es la de salida, la cual se compone de $1000$ neuronas que poseen una función de activación \textit{softmax}, esta capa fue modificada para que sea $4$ neuronas de salida referentes a las 4 clases definidas. Estas neuronas son las encargadas de realizar la clasificación de la imagen. 
...
\begin{figure}[ht]
	...
	\caption{Arquitectura Del Modelo \textit{AlexNet} \\ Fuente:Alex Krizhevsky (2014)}
	\label{fig:AlexNet}
\end{figure}

Como se aprecia en la Figura \ref{fig:AlexNet}, el modelo de \textit{AlexNet} toma una imagen de entrada con una resolución de \textit{227 x 227} pixeles y $3$ canales de color, la información recibida pasa por cinco capas convolucionales (en negro) de neuronas con función de activación \textit{ReLu} y $3$ capas de \textit{Max pooling} (en azul). La quinta capa de convolución posee $256$ máscaras diferentes con dimensiones de \textit{13 x 13}. Seguidamente se aplican las dos capas densas de $4096$ neuronas, las cuales son seguidas por una capa de salida de $1000$ neuronas, dotadas con una función de activación \textit{softmax}. Cada neurona de la capa de salida representa una clase diferente y su valor de activación indica la probabilidad de que la imagen de entrada pertenezca a dicho conjunto semántico. Los resultados del entrenamiento obtenidos fueron inferiores al $20\%$, por lo tanto, fueron excluidos en el análisis sin optimización de los hiperparámetros.

\subsubsection{\MakeUppercase{Modelo preentrenado VGG19}}

La arquitectura \textit{VGG} fue planteada por Simonyan y Zisserman \cite{simonyan2014very}, la cual consiste en grupos lineales de bloques que se encuentran formados por una cantidad determinada de capas convolucionales, una función de activación no lineal y una capa de \textit{Max pooling}, seguidos de tres capas densas y finalmente una capa de activación \textit{softmax} \cite{ref_2}. Esta arquitectura cuenta con $5$ bloques los cuales están distribuidos como se muestra en la Figura \ref{fig:VGG19}.				

\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.45]{Figs/22.jpeg}
	\caption{Arquitectura Del Modelo VGG \\ Fuente: Karen Simonya (2015)}
	\label{fig:VGG19}
\end{figure}			


Como se puede observar en la Figura \ref{fig:VGG19}, la columna \textit{E} muestra que la arquitectura del modelo \textit{VGG19}  cuenta con $2$ capas convolucionales de $64$ y $128$ filtros respectivamente, en sus dos primeros bloques, el bloque intermedio se compone de $3$ capas convolucionales de $256$ filtros, y los últimos dos bloques están compuestos por $3$ capas convolucionales de $512$ filtros cada uno. El numero $19$ representa la cantidad de capas que se pueden entrenar en la arquitectura, donde hay $16$ capas convolucionales y $3$ capas densas.\\			

Las capas convolucionales de esta arquitectura presentadas en la Figura \ref{fig:VGG19}, cuentan con un campo receptivo de \textit{3 x 3}, \textit{stride} de \textit{1 x 1} y \textit{padding} de $1$ pixel. Para la operaciones de \textit{Max pooling} se implementa un \textit{kernel} de \textit{2 x 2} y un \textit{stride} de \textit{2 x 2}, y por ultimo cada capa oculta de la red cuenta con la función de activación \textit{ReLu}. La Figura \ref{fig:VGG19_accuracy} muestra la precisión obtenida del entrenamiento hecho sobre esta arquitectura.

\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.61]{Figs/101.png}
...
\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.61]{Figs/102.png}
...
\begin{table}[htbp]
	...
	\resizebox{10cm}{!} {
		\begin{tabular}{|c|l|c|c|c|c|}
			...
			\cline{3-6}    \multicolumn{2}{|c|}{} & CLASE 1 & CLASE 2 & CLASE 3 & CLASE 4 \bigstrut\\
			\hline
			\multirow{4}[8]{*}{\begin{sideways}Valor Real\end{sideways}} & CLASE 1 & 26     & 3     & 0    & 0 \bigstrut\\
			\cline{2-6}     & CLASE 2 & 17     & 7     & 0    & 0 \bigstrut\\
			\cline{2-6}      & CLASE 3 & 6     & 15     & 0    & 1 \bigstrut\\
			\cline{2-6}     & CLASE 4 & 19     & 21     & 0    & 3 \bigstrut\\
			\hline
		\end{tabular}
	}
...
\begin{table}[htbp]
	...
	\begin{tabular}{|c|c|}
		\hline
		CLASE 1 & 89.7 \bigstrut\\
		\hline
		CLASE 2 & 29.2\bigstrut\\
		\hline
		CLASE 3 & 0.0 \bigstrut\\
		\hline
		CLASE 4 & 7.0 \bigstrut\\
		\hline
		\multicolumn{2}{|c|}{Precisión Total: 34.21} \bigstrut\\
		\hline
	\end{tabular}%
...
La arquitectura \textit{ResNet} (Residual Net), fue propuesta por un equipo de Microsoft Research liderado por \textit{Kaiming He} \cite{he2016deep}, en donde se propone que si a una red neuronal de $20$ capas se le intercalan $36$ capas que calculan la simple función de identidad, la red resultante debería tener exactamente la misma eficacia y no ser inferior que la primera. Se debe recordar que la función identidad es la encargada de devolver exactamente el mismo valor que su argumento \cite{ref_4}.\\

Conociendo esto para permitir que una red residual pueda variar su cantidad efectiva de capas, se introduce el concepto de bloque residual:

\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.7]{Figs/67.png}
	\caption{Bloque Residual \\ Fuente: Kaiming He (2015) }
	\label{fig:Bloque}
\end{figure}
...
\begin{figure}[ht]
	...
	\caption{Arquitectura \textit{ResNet18} \\ Fuente: Kaiming He (2015)}
	\label{fig:ArqRes18}
\end{figure}
...
\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.66]{Figs/109.png}
	\caption{Precisión Obtenida Con Modelo \textit{Resnet18}}
...
\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.66]{Figs/110.png}
	\caption{Pérdidas Obtenidas Con Modelo \textit{Resnet18}}
...
Para comprara los resultados posteriormente, se calcula la matriz de confusión con el conjunto de datos de \textit{entrenamiento}, los resultados se presentan en la Tabla \ref{MC_RESNET18_2}.
...
\begin{table}[htbp]
	...
	\begin{tabular}{|c|c|}
		...
		CLASE 4 & 0,00 \bigstrut\\
		\hline
		\multicolumn{2}{|c|}{Overall Accuracy: 29.00} \bigstrut\\
		\hline
	\end{tabular}%
	\caption{Precisión por clase en conjunto de \textit{validación}}
...
La Tabla \ref{tab:MC_ALEX_OPT} almacena los datos de la matriz de confusión del modelo \textit{AlexNet} en el conjunto de datos de \textit{validación}.
...
\newpage
La Tabla \ref{tab:Alexoptclases} presenta la precisión del modelo por cada clase de clasificación en el conjunto de datos de \textit{validación}.
